...
def problem3a(window, point, n):
    ...
    total = 0
    for k in range(n):
        line = rg.Line(rg.Point(x + xInc * k, y + yInc * k), rg.Point(x + xInc * k, y + yInc * k + length))
        line.attach_to(window)
        line.thickness = thickSum
        window.render()
        total += thickSum
        if thickSum < 13:
            thickSum += 2
...
def problem3b(m, point1):
    ...
    total = 0
    for k in range(m):
        for l in range(lineNum+1):
            problem3a(window, rg.Point(point1.x, point1.y + 60*k), l)
        lineNum += 2
        total += problem3a(window, rg.Point(point1.x, point1.y + 60*k), l)
    window.render()
